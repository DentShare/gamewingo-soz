...
export interface FinishInput { level: LevelId; hints: number; durationMs: number; }
...
export function createSession(
  ...
  return {
    ...
    async finish(input) {
      ...
      bridge.track('round_finished', { level: input.level, hints: input.hints });
      if (!api) return null;
      try {
        return await api.submitScore({
          sessionId, gameId: 'sudoku-kids', score, durationMs: input.durationMs,
          meta: { level: input.level, hints: input.hints },
        });
      } catch (err) {
    ...
    async leaderboard(limit = 10) {
      if (!api) return [];
      try {
        return await api.leaderboard('sudoku-kids', limit);
      } catch (err) {
        bridge.error(String(err));
        return [];
      }
