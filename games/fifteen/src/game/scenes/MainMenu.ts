...
export class MainMenu extends Scene {
  ...
  create() {
    ...
    this.add
      .text(CX, 94, '🧩', { fontFamily: FONT, fontSize: 40 })
      .setOrigin(0.5);
    this.add
  ...
  /** «Как играть» — интерактивное обучение на настоящем поле 3×3; по концу → обратно в меню. */
  private showHowto() {
    this.registry.set('howto', true);
    this.registry.set('locale', this.locale);
    this.registry.set('level', '3x3'); // детское поле нагляднее для обучения
    this.scene.start('Game');
  }
}
