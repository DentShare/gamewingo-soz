...
export class MainMenu extends Scene {
  ...
  create() {
    ...
    // Что внутри: шесть тем — главная причина зайти. Ниже сводки лестницы,
    // которая занимает две строки (заголовок и счётчик звёзд).
    this.add
      .text(CX, 182, t(this.locale, 'menu.topics'), {
        fontFamily: FONT, fontSize: 12, color: COLORS.headMuted,
        align: 'center', wordWrap: { width: 340 }, lineSpacing: 2,
      })
      .setOrigin(0.5, 0)
      .setResolution(DPR);
    ...
    const grid = makeLevelGrid(this, CX, 226, tiles, (n) => this.startLevel(n));

    const belowGrid = 226 + grid.height + 30;
  ...
  /** «Как играть» — обучение поверх настоящей партии; по концу → в меню. */
  private showHowto() {
