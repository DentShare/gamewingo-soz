...
/** Прямоугольник в логических координатах сцены (ширина 400). */
...
export interface OnboardingTargets {
  /** Вся доска вместе с плашками сумм. */
  board: Rect;
  /** Одна настоящая строка вместе с её целевой суммой. */
  row: Rect;
  /** Клетка, на которой показываем вычёркивание. */
  cell: Rect;
  /** Кнопка «Заново». */
  reset: Rect;
}

/** Демонстрация вычёркивания на реальной доске (управляет состоянием сцены Game). */
export interface OnboardingDemo {
  /** Зачеркнуть демонстрационную клетку — ровно как при тапе игрока. */
  cross(): void;
  /** Откатить демонстрацию: доска игрока остаётся нетронутой, ходы не считаются. */
  reset(): void;
}

const W = 400;
/**
 * Границы видимой области, а не логические 720: кнопка «Заново» стоит ниже 720
 * на вытянутом экране, и по старым границам подсветка обрезалась в тонкую полоску.
 */
const TOP = VIEW_TOP;
...
interface Step {
  ...
  pad: number;
  radius: number;
  /** Шаг с настоящим вычёркиванием клетки. */
  demo?: boolean;
}

const STEPS: Step[] = [
  { holes: ['board'], textKey: 'onboarding.board', pad: 6, radius: 12 },
  { holes: ['row'], textKey: 'onboarding.row', pad: 4, radius: 8 },
  { holes: ['cell'], textKey: 'onboarding.cross', pad: 5, radius: 10, demo: true },
  { holes: ['reset'], textKey: 'onboarding.reset', pad: 6, radius: 16 },
];
const TOTAL_STEPS = STEPS.length;

/**
 * Пошаговое обучение поверх настоящего игрового поля: затемняет экран, по очереди
 * подсвечивает доску, строку с её суммой, вычёркивание и кнопку сброса. Игровой ввод
 * на время обучения перехватывается прозрачным блокером. Одноразовость — на стороне
 * вызывающего.
 */
...
class Onboarding {
  ...
  show(step: number): void {
    ...
    const blocker = this.scene.add.rectangle(0, TOP, W, BOTTOM - TOP, 0, 0).setOrigin(0, 0).setInteractive();
  ...
  /** Настоящее вычёркивание на доске: клетка гаснет и перечёркивается сама. */
  private startDemo(): void {
    this.demoActive = true;
    this.demoTimer = this.scene.time.delayedCall(520, () => this.demo.cross());
  }
  ...
  private dimAround(layer: Phaser.GameObjects.Container, holes: Rect[]): void {
    ...
    strip(0, cursor, W, BOTTOM - cursor); // под последней дыркой
  }
  ...
  /** «Пропустить» — под линией HUD, чтобы не наезжать на счётчик ходов. */
...
/** Расширяет прямоугольник на `p` и обрезает по видимой области. */
function padClamp(r: Rect, p: number): Rect {
  const x = Math.max(0, r.x - p);
  const y = Math.max(TOP, r.y - p);
  const right = Math.min(W, r.x + r.w + p);
  const bottom = Math.min(BOTTOM, r.y + r.h + p);
...
function cardTopFor(holes: Rect[], cardH: number): number {
  ...
  bands.push({ top: cursor, h: BOTTOM - cursor });
  ...
  return Math.round(Math.min(Math.max(top, TOP + 8), BOTTOM - 8 - cardH));
}
