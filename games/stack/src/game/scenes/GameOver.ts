...
export class GameOver extends Scene {
  ...
  create() {
    ...
    const round = recordArcadeRound({
      ...
      defs: CHALLENGES,
      metrics: {
        blocks: last.blocks, perfects: last.perfects,
        perfectStreak: last.perfectStreak, score: last.score,
      },
      score: last.score,
    });
    ...
    this.appear(
      this.add
        .text(CX, 182, `${t(loc, 'result.detail', { blocks: last.blocks, perfects: last.perfects })}${isRecord ? ` · ${t(loc, 'result.newBest')}` : ''}`, {
