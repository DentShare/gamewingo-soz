...
export interface OnboardingStep {
  ...
  radius?: number;
  /** Демонстрация на реальном поле перед показом подсказки (например, перетаскивание). */
  prepare?: () => void;
  /**
   * Принудительный центр карточки-подсказки по Y. Нужен, когда демонстрация
   * (`prepare`) ездит по экрану и карточка иначе перекрыла бы её траекторию.
   */
  captionY?: number;
}
...
class Onboarding {
  ...
  show(step: number): void {
    ...
    const showCaption = () => {
      ...
      this.captionCard(layer, hole, t(this.locale, cfg.textKey), step, cfg.captionY);
    };
  ...
  private captionCard(
    ...
    const cy = forcedY ?? placeCaption(hole, h);
    const top = cy - h / 2;
