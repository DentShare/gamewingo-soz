...
export interface OnboardingStep {
  ...
  radius?: number;
  /** Демонстрация на реальном поле перед показом подсказки (например, зажечь цель). */
  prepare?: () => void;
}
 ...
 * заранее менять поле (`prepare`) — так правила показываются на реальных целях.
 ...
 * Если зона занимает почти весь экран — прижимаем карточку вниз.
 */
