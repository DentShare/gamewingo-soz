...
export const GAME_ID = 'flyer';

export interface FinishInput {
  /** Готовый счёт (см. `core/score.ts`) — сервер валидирует его заново. */
  score: number;
  /** Пройденные проёмы — метрика для антифрода. */
  passed: number;
  durationMs: number;
}
...
export function createSession(
  ...
  return {
    ...
    async leaderboard(limit = 10) {
      if (!api) return [];
      try {
        return await api.leaderboard(GAME_ID, limit);
      } catch (err) {
        bridge.error(String(err));
        return [];
      }
