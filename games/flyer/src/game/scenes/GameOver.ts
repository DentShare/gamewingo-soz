...
export class GameOver extends Scene {
  ...
  create() {
    ...
    // Запись полёта: испытания каскадом, рекорды, счётчики дня.
    ...
    this.appear(
      this.add
        .text(CX, 182, `${t(loc, 'result.detail', { passed: last.passed, time })}${isRecord ? ` · ${t(loc, 'result.newBest')}` : ''}`, {
    ...
    // Закрытые этим полётом испытания — по строке с тремя звёздами.
    let y = 236;
