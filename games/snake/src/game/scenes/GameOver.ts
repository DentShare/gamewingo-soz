...
export class GameOver extends Scene {
  ...
  create() {
    ...
    const round = recordArcadeRound({
      ...
      defs: CHALLENGES,
      metrics: {
        eaten: last.eaten, lengthMax: last.lengthMax,
        survivedSec: last.survivedSec, feast12: last.feast12,
      },
      score: last.score,
    });
    ...
    this.appear(
      this.add
        .text(CX, 182, `${t(loc, 'result.length', { n: last.lengthMax })}${isRecord ? ` · ${t(loc, 'result.newBest')}` : ''}`, {
