...
interface LastGame {
  locale: Locale; correct: number; mistakes: number; durationMs: number;
}
...
export class GameOver extends Scene {
  ...
  create() {
    ...
    const isNewBest = saveBest({ score, correct: last.correct, mistakes: last.mistakes });
    ...
    const title = this.add
      .text(CX, 108, t(loc, 'result.title'), {
        fontFamily: FONT, fontSize: 36, color: COLORS.headText, fontStyle: 'bold',
      })
      .setOrigin(0.5)
      .setResolution(DPR)
      .setScale(0.7)
    ...
    this.appear(
      this.add
        .text(CX, 268, t(loc, 'result.score', { score }), {
    ...
    this.appear(
      this.add
        .text(CX, 304, t(loc, 'result.mistakes', { n: last.mistakes }), {
    ...
    const again = makeButton(this, CX, 404, t(loc, 'result.playAgain'), () => this.scene.start('Game'), {
      primary: true, width: 260, height: 60,
    });
    this.appear(again.root, 700);
    const menu = makeButton(this, CX, 468, t(loc, 'result.menu'), () => this.scene.start('MainMenu'));
    this.appear(menu.root, 770);

    // Лидерборд.
    const lbTitle = this.add
      .text(CX, 528, t(loc, 'result.leaderboard'), {
        fontFamily: FONT, fontSize: 17, color: COLORS.headText, fontStyle: 'bold',
      })
      .setOrigin(0.5)
      .setResolution(DPR);
    const listText = this.add
      .text(CX, 554, '…', { fontFamily: FONT, fontSize: 15, color: COLORS.headMuted, align: 'center' })
      .setOrigin(0.5, 0)
      .setResolution(DPR);
