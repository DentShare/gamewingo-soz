...
/** Перечёркивание: жирная полоса на грани — «это число вычеркнуто». */
const strike = (x, y, z, len = 58, fill = '#FF7A00') =>
  shape(x, y, z, [[-len, -9], [len, -9], [len, 9], [-len, 9]], fill);
...
const ICONS = {
  ...
  // Суммы: доска 2×2, два числа оставлены, два вычеркнуты
  sums: () => [
    shadow(0, 0, 190, 62),
    plate(-190, -190, 380, 380, 20),
    box(-170, -170, 20, 160, 160, 42, ORANGE),
    label(-90, -90, 62, '5', 84),
    box(10, -170, 20, 160, 160, 42, WHITE),
    label(90, -90, 62, '2', 78, '#B9C3C7'),
    strike(90, -90, 62),
    box(-170, 10, 20, 160, 160, 42, WHITE),
    label(-90, 90, 62, '4', 78, '#B9C3C7'),
    strike(-90, 90, 62),
    box(10, 10, 20, 160, 160, 42, TEAL),
    label(90, 90, 62, '7', 84),
  ],
  ...
  // Викторина: карточка с вопросом и два варианта — верный (галка) и обычный
  quiz: () => [
    shadow(0, 0, 190, 62),
    plate(-195, -175, 390, 350, 20),
    box(-150, -140, 20, 300, 110, 62, TEAL),
    label(0, -85, 82, '?', 92),
    box(-150, -10, 20, 300, 78, 44, ORANGE),
    decal(0, 29, 64, `
      <path d="M-26 2 L-8 20 L28 -18" fill="none" stroke="#FFFFFF"
        stroke-width="16" stroke-linecap="round" stroke-linejoin="round"/>`, 30),
    box(-150, 92, 20, 300, 78, 44, WHITE),
    decal(0, 131, 64, `<rect x="-70" y="-8" width="140" height="16" rx="8" fill="#C9D2D6"/>`, 72),
  ],

  // Пазл: собранная картинка из четырёх кусочков, один приподнят и встаёт на место
  jigsaw: () => [
    shadow(0, 0, 190, 62),
    plate(-190, -190, 380, 380, 18),
    recess(-170, -170, 18, 340, 340),
    box(-160, -160, 18, 160, 160, 40, TEAL),
    box(0, -160, 18, 160, 160, 40, ORANGE),
    box(-160, 0, 18, 160, 160, 40, ORANGE_D),
    // Четвёртый кусочек ещё в воздухе — видно, что картинку собирают.
    box(10, 20, 120, 150, 150, 40, TEAL_L),
    decal(85, 95, 160, `<circle cx="0" cy="0" r="26" fill="#FFFFFF" opacity=".9"/>`, 28),
  ],
};
