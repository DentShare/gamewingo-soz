...
| quiz (викторина) | github.com/phaserjs/template-vite-ts (через структуру counting); механика реализована с нуля | MIT | сохранён `games/quiz/LICENSE` + копирайт Phaser Studio | 2026-08-08 | — |
...
| Картинки пазла (временные) | собственная векторная отрисовка (`games/jigsaw/src/game/painters.ts`) | — (собственная) | без сторонних прав; плейсхолдер до покупки набора | jigsaw | 2026-08-08 | — |
| Картинки пазла (набор) | **не выбран** — ждёт покупки, см. `games/jigsaw/SPEC.md` §3 | — | ДО подключения: проверить запрет на редистрибуцию, число проектов, атрибуцию; записать строкой сюда | jigsaw | — | — |
| Звуки интерфейса (8 эффектов) | kenney.nl/assets/interface-sounds, пак «Interface Sounds 1.0»; мастер-копия и файл лицензии — `packages/game-ui/audio/` | CC0 1.0 | без условий, атрибуция не требуется; перекодированы в моно mp3 (`ffmpeg -ac 1 -ar 44100 -b:a 96k -c:a libmp3lame`) и зашиты в бандл base64 (`scripts/build-audio.mjs`) | все игры каталога | 2026-08-08 | — |
|  |  |  |  |  |  |  |
